...
int avs_cldma_load_basefw(struct avs_dev *adev, struct firmware *fw)
{
	...
	ret = avs_dsp_op(adev, power, spec->master_mask, true);
	if (ret < 0) {
		dev_err(adev->dev, "core_mask %d power on failed: %d\n",
			spec->master_mask, ret);
		return ret;
	}

	ret = avs_dsp_op(adev, reset, spec->master_mask, false);
	if (ret < 0) {
		dev_err(adev->dev, "core_mask %d unreset failed: %d\n",
			spec->master_mask, ret);
		return ret;
	}
	...
	ret = avs_dsp_op(adev, stall, spec->master_mask, false);
	if (ret < 0) {
		dev_err(adev->dev, "core_mask %d unstall failed: %d\n",
			spec->master_mask, ret);
		return ret;
	}
	...
	if (ret < 0) {
		...
		avs_dsp_core_disable(adev, spec->master_mask);
		return ret;
	}
...
int avs_cldma_load_library(struct avs_dev *adev, struct firmware *lib, u32 id)
{
	...
	hda_cldma_transfer(cl, msecs_to_jiffies(100));
...
avs_hda_init_rom(struct avs_dev *adev, unsigned int dma_id, bool purge)
{
	...
	corex_mask = spec->init_mask & ~(spec->master_mask);

	ret = avs_dsp_op(adev, power, spec->init_mask, true);
	if (ret < 0) {
		dev_err(adev->dev, "core_mask %d power up failed: %d\n",
			spec->init_mask, ret);
		goto err;
	}
...
err:
	avs_dsp_core_disable(adev, spec->init_mask);
	return ret;
}
...
static int avs_imr_load_basefw(struct avs_dev *adev)
{
	...
	if (!ret) {
		...
		avs_dsp_core_disable(adev, adev->spec->master_mask);
		return -ETIMEDOUT;
	}
...
int avs_hda_load_basefw(struct avs_dev *adev, struct firmware *fw)
{
	...
	ret = snd_hdac_dsp_prepare(stream, sdfmt, fw->size, &dmab);
	if (ret < 0)
		goto release_stream;
	...
	for (i = 0; i < AVS_ROM_INIT_RETRIES; i++) {
		unsigned int dma_id = stream->stream_tag - 1;
...
static int avs_dsp_load_basefw(struct avs_dev *adev)
{
	...
	ret = avs_dsp_op(adev, load_fw, &stripped_fw);
	if (ret < 0) {
...
int avs_dsp_first_boot_firmware(struct avs_dev *adev)
{
	...
	if (avs_platattr_test(adev, CLDMA)) {
		ret = hda_cldma_init(&code_loader, &adev->bus.core,
				      adev->adsp_ba, CL_DEFAULT_BUFFER_SIZE);
		if (ret < 0) {
