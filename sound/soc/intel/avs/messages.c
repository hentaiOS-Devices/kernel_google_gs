			       ...
			       enum avs_pipeline_state *state)
{
	...
	struct avs_ipc_msg request = {0}, reply = {0};
	int ret;
			     ...
			     u8 *request, size_t request_size)
{
	...
	/* Loop the rest only when payload when exceeds mailbox's size. */
	while (remaining) {
		size_t offset;
			     ...
			     u8 **reply_data, size_t *reply_size)
{
	...
	struct avs_ipc_msg request, reply = {0};
	size_t size;
	void *buf;
	int ret;
...
int avs_ipc_get_hw_config(struct avs_dev *adev, struct avs_hw_cfg *cfg)
{
	...
	while (offset < payload_size) {
		...
		default:
			dev_info(adev->dev, "Unrecognized hw param: %d\n",
				tlv->type);
			break;
		}
