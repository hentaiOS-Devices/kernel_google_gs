...
static int avs_create_dai_links(struct device *dev,
				struct hda_codec *codec, int pcm_count,
...
static int avs_create_dapm_routes(struct device *dev,
				  struct hda_codec *codec, int pcm_count,
...
static int avs_probing_link_init(struct snd_soc_pcm_runtime *rtm)
{
	...
	ret = avs_create_dai_links(card->dev, codec, pcm_count,
				   mach->mach_params.platform, &links);
	if (ret < 0) {
...
MODULE_DESCRIPTION("Generic Intel HD-Audio machine driver");
