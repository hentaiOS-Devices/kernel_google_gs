...
static void rk3399_vpu_vp8_enc_set_buffers(struct hantro_dev *vpu,
					   struct hantro_ctx *ctx)
{
	...
	ctx->vp8_enc.buf_data.hdr_size = params->frm_hdr_size;
	...
	vepu_write_relaxed(vpu, ctx->vp8_enc.priv_src.dma + VP8_CABAC_CTX_OFFSET,
			   VEPU_REG_ADDR_CABAC_TBL);

	/* XXX is this memset needed? */
	memset(ctx->vp8_enc.priv_src.cpu + VP8_CABAC_CTX_OFFSET + VP8_CABAC_CTX_SIZE,
	       0, VP8_SEGMENT_MAP_SIZE);
	vepu_write_relaxed(vpu, ctx->vp8_enc.priv_src.dma + VP8_CABAC_CTX_OFFSET +
					VP8_CABAC_CTX_SIZE,
...
static void rk3399_vpu_vp8_enc_set_params(struct hantro_dev *vpu,
					  struct hantro_ctx *ctx)
{
	const struct rk3399_vp8_enc_reg_params *params =
		hantro_get_ctrl(ctx, V4L2_CID_PRIVATE_HANTRO_REG_PARAMS);
...
void rk3399_vpu_vp8_enc_run(struct hantro_ctx *ctx)
{
	...
	/* Used in rk3399_vpu_vp8_enc_done() */
	ctx->vp8_enc.params = params;
	 ...
	 */
	reg = VEPU_REG_ENCODE_FORMAT_VP8;
	vepu_write_relaxed(vpu, reg, VEPU_REG_ENCODE_START);

	rk3399_vpu_vp8_enc_set_params(vpu, ctx);
	rk3399_vpu_vp8_enc_set_buffers(vpu, ctx);
...
void rk3399_vpu_vp8_enc_done(struct hantro_ctx *ctx)
{
	...
	/*
	 * hantro_get_ctrl() can't be used, as the underlying v4l2_ctrl_find()
	 * acquires the mutex in |struct v4l2_ctrl_handler|, and this function
	 * is called from the interrupt handler in interrupt context.
	 */
	const struct rk3399_vp8_enc_reg_params *params = ctx->vp8_enc.params;
