...
struct hantro_h264_enc_feedback {
	s32 qp_sum;
	s32 cp[10];
	s32 mad_count;
	s32 rlc_count;
...
static const s32 h264_context_init[H264E_CABAC_IDC_NUM][HANTRO_H264_CABAC_CV_NUM][2] = {
	/* cabac_init_idc == 0 */
	{
		/* 0 -> 10 */
		{ 20, -15 }, { 2, 54 }, { 3, 74 }, { 20, -15 },
...
static void rk3399_vpu_h264_enc_init_cabac_table(struct hantro_ctx *ctx)
{
	u8 *table;
	...
	for (k = 0; k < H264E_CABAC_IDC_NUM; k++) {
		table = (u8 *)ctx->h264_enc.cabac_tbl[k].cpu;
		for (qp = 0; qp < HANTRO_H264_QP_NUM; qp++) { /* All QP values */
		...
		buffer_cpu_to_be64((u64 *)table,
				   HANTRO_H264_CABAC_TABLE_SIZE / sizeof(u64));
	}
}
...
int rk3399_vpu_h264_enc_init(struct hantro_ctx *ctx)
{
	...
	for (i = 0; i < H264E_CABAC_IDC_NUM; i++) {
		ret = hantro_aux_buf_alloc(vpu, &ctx->h264_enc.cabac_tbl[i],
					   HANTRO_H264_CABAC_TABLE_SIZE);
		if (ret) {
			vpu_err("allocate h264_enc cabac_tbl failed\n");
			goto err_ext_buf_alloc;
		}
	}

	ref_buf_size = hantro_rounded_luma_size(width, height) * 3 / 2;
	ret = hantro_aux_buf_alloc(vpu, &ctx->h264_enc.ext_buf,
				   2 * ref_buf_size);
	if (ret) {
	...
	ret = hantro_aux_buf_alloc(vpu, &ctx->h264_enc.feedback,
				   HANTRO_VP8_RET_PARAMS_SIZE);
	if (ret) {
		vpu_err("allocate feedback failed\n");
		goto err_feedback_alloc;
	}

	rk3399_vpu_h264_enc_init_cabac_table(ctx);

	return ret;

err_feedback_alloc:
	hantro_aux_buf_free(vpu, &ctx->h264_enc.ext_buf);

err_ext_buf_alloc:
	for (i = 0; i < H264E_CABAC_IDC_NUM; i++)
		if (ctx->h264_enc.cabac_tbl[i].size != 0)
			hantro_aux_buf_free(vpu, &ctx->h264_enc.cabac_tbl[i]);
...
void rk3399_vpu_h264_enc_exit(struct hantro_ctx *ctx)
{
	...
	hantro_aux_buf_free(vpu, &ctx->h264_enc.feedback);
	hantro_aux_buf_free(vpu, &ctx->h264_enc.ext_buf);
...
static void rk3399_vpu_h264_enc_set_buffers(struct hantro_dev *vpu,
					    struct hantro_ctx *ctx)
{
	...
	size_t rounded_size;
	dma_addr_t cabac_dma, dst_dma;
	int i;

	src_buf = hantro_get_src_buf(ctx);
	dst_buf = hantro_get_dst_buf(ctx);

	cabac_dma = ctx->h264_enc.cabac_tbl[params->cabac_init_idc].dma;
	...
	vepu_write_relaxed(vpu, vb2_plane_size(&dst_buf->vb2_buf, 0),
	...
	rounded_size = hantro_rounded_luma_size(ctx->src_fmt.width,
						ctx->src_fmt.height);
	rec_buf_dma = ctx->h264_enc.ext_buf.dma;
	ref_buf_dma = rec_buf_dma;
	if (ctx->h264_enc.ref_rec_ptr)
		rec_buf_dma += rounded_size * 3 / 2;
	else
		ref_buf_dma += rounded_size * 3 / 2;
	ctx->h264_enc.ref_rec_ptr ^= 1;
...
static void rk3399_vpu_h264_enc_set_params(struct hantro_dev *vpu,
					   struct hantro_ctx *ctx)
{
	const struct hantro_h264_enc_reg_params *params =
		hantro_get_ctrl(ctx, V4L2_CID_PRIVATE_HANTRO_REG_PARAMS);
...
void rk3399_vpu_h264_enc_run(struct hantro_ctx *ctx)
{
	...
	rk3399_vpu_h264_enc_set_params(vpu, ctx);
	rk3399_vpu_h264_enc_set_buffers(vpu, ctx);
...
{
	struct hantro_dev *vpu = ctx->dev;
	struct hantro_h264_enc_feedback *feedback = ctx->h264_enc.feedback.cpu;
	...
	slice_size = vepu_read(vpu, VEPU_REG_STR_BUF_LIMIT) / 8;
	vb2_set_plane_payload(&dst_buf->vb2_buf, 0, slice_size);
}
