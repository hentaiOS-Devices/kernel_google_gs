...
static void kbase_jd_post_external_resources(struct kbase_jd_atom *katom)
{
	...
	if (katom->extres) {
		...
		while (res_no-- > 0) {
			kbase_unmap_external_resource(katom->kctx,
						      katom->extres[res_no]);
		}
		kfree(katom->extres);
		katom->extres = NULL;
...
static int kbase_jd_pre_external_resources(struct kbase_jd_atom *katom, const struct base_jd_atom *user_atom)
{
	...
	input_extres = kmalloc_array(katom->nr_extres, sizeof(*input_extres),
				     GFP_KERNEL);
	if (!input_extres) {
		err = -ENOMEM;
	...
	if (copy_from_user(input_extres,
			   get_compat_pointer(katom->kctx,
					      user_atom->extres_list),
	...
	for (res_no = 0; res_no < katom->nr_extres; res_no++) {
...
#ifdef CONFIG_MALI_VALHALL_DMA_FENCE
		bool exclusive;
		exclusive = (user_res->ext_resource &
			     BASE_EXT_RES_ACCESS_EXCLUSIVE) ?
			    true : false;
#endif
		reg = kbase_region_tracker_find_region_enclosing_address(
			katom->kctx, user_res->ext_resource &
					     ~BASE_EXT_RES_ACCESS_EXCLUSIVE);
		...
		err = kbase_map_external_resource(katom->kctx, reg,
						  current->mm);
		if (err)
			goto failed_loop;
